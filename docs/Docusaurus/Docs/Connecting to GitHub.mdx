...
<p>Go to `Settings` and then `Secrets and Variables > Actions` and create a new Repository Secret</p>
![GitLeaks Secret](/img/Docusaurus/GitLeaks_Secrets.png)
<p>For the name, type in `GIT_TOKEN`</p>
Paste your GitHub API Key into the secret field and then save.

#### Creating the GitLeaks Workflow Action
<p>Go to `Actions` and then Click `New Workflow`</p>

![GitLeaks](/img/Docusaurus/GitLeaks1.png)

<p>Set up a New Workflow</p>
