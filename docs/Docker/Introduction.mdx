...
services:
  ...
  database:
    ...
    network_mode: "service:web"
```
Using service:web means the container will share the exact same network namespace as the "web" service. This includes:
- Same IP address
- Same network interfaces
- Same port space
- Same DNS configuration
- This is useful for scenarios like sidecar containers that need to monitor or interact with the main service's network traffic.
...
services:
  app:
    image: my-app:image
    network_mode: "container:webapp-1"
...
- Share network resources with an existing container
- Monitor network traffic of another container
- Debug network-related issues
- Implement network-level tools like proxies or monitoring agents
