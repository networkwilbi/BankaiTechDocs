...
server {
    ...
    set $upstream_adguard http://192.168.4.55;
    #              Logging
    ...
    location / {
        ...
        proxy_redirect / /;
        proxy_cookie_path / /;
        proxy_pass $upstream_adguard:3001;
        #websockets
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection "upgrade";
    }
...
## HomeBridge
```nginx title="Highlighted items will need to be modified" {8,10,18,20,21,22} showLineNumbers
map $http_upgrade $connection_upgrade {
    default upgrade;
    '' close;
}
...
server {
    ...
    location / {
        ...
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection "upgrade";
        proxy_pass http://192.168.4.200:8581;
        proxy_read_timeout 300s;
        #Cache
        proxy_buffering on;
        proxy_cache_valid 200;
...
server {
    ...
    location / {
        ...
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection "upgrade";
        proxy_pass http://192.168.4.207:8123;
        proxy_read_timeout 300s;
        #Cache
        proxy_buffering on;
        proxy_cache_valid 200;
...
server {
    ...
    set $jellyfin 192.168.4.31:8096;
    ssl_stapling on;
    ssl_stapling_verify on;
    resolver 1.1.1.1 1.0.0.1;
    #                                            Load TLS Certs
...
server {
    ...
    location / {
        ...
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection "upgrade";
        proxy_pass http://192.168.4.206:5055;
        proxy_read_timeout 300s;
    }
}
```
